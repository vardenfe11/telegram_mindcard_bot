...
class Bot:
    ...
    def repeat_cards(self, update: Update, context: CallbackContext, button=None):
        ...
        if button:                              # вызов из inline-кнопки
            ...
            if btn_action in ('remember', 'forgot'):
                user.state = 'repeat'
                user.save()
                ...
                if card:
                    ...
                    if btn_action == 'forgot':
                        card.repeat_mistake += 1
                    self.user_card[user.user_id] = user.get_card(self.db)
            elif btn_action == 'delete' and self.user_card[user.user_id]:
                user.state = 'delete'
            ...
            elif btn_action in ('yes', 'no'):
                if btn_action == 'yes' and self.user_card[user.user_id].card_id == int(button[1]):
                    self.card_delete(user)
                user.state = 'repeat'
                self.user_card[user.user_id] = user.get_card(self.db)
            elif btn_action in ('listenback', 'listenfront'):
                user.state = 'repeat'
                user.save()
